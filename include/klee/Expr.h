...
public:
  static ref<Expr> alloc(llvm::Value* _address, std::string _name, const ref<Expr> &_index) {
    ref<Expr> r(new WPVarExpr(_address, _name, _index));
    r->computeHash();
    return r;
  }

  static ref<Expr> create(llvm::Value* address, std::string name, ref<Expr> index);
...
private:
  WPVarExpr(llvm::Value* _address, std::string _name, const ref<Expr> &_index) :
	  address(_address), name(_name) {index = _index;}

public:
  static bool classof(const Expr *E) {
    return E->getKind() == Expr::WPVar;
  }
