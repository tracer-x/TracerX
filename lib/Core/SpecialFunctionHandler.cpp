...
void SpecialFunctionHandler::handleAliasFunction(ExecutionState &state,
  ...
  std::string old_fn = readStringAtAddress(state, arguments[0].value);
  std::string new_fn = readStringAtAddress(state, arguments[1].value);
...
void SpecialFunctionHandler::handleAssert(ExecutionState &state,
  ...
  executor.terminateStateOnError(
      state,
      "ASSERTION FAIL: " + readStringAtAddress(state, arguments[0].value),
      Executor::Assert);
}
...
void SpecialFunctionHandler::handleReportError(ExecutionState &state,
  ...
  executor.terminateStateOnError(
      state, readStringAtAddress(state, arguments[2].value),
      Executor::ReportError,
      readStringAtAddress(state, arguments[3].value).c_str());
}
...
void SpecialFunctionHandler::handleIsSymbolic(ExecutionState &state,
  ...
  executor.bindLocal(
      target, state,
      ConstantExpr::create(!isa<ConstantExpr>(arguments[0].value), Expr::Int32),
      arguments[0].vvalue);
}
...
void SpecialFunctionHandler::handlePosixPreferCex(
    ExecutionState &state, KInstruction *target, std::vector<Cell> &arguments) {
  if (ReadablePosix)
...
void SpecialFunctionHandler::handlePrintExpr(ExecutionState &state,
  ...
  std::string msg_str = readStringAtAddress(state, arguments[0].value);
...
void SpecialFunctionHandler::handlePrintRange(ExecutionState &state,
  ...
  if (!isa<ConstantExpr>(arguments[1].value)) {
    ...
    bool success __attribute__((unused)) =
        executor.solver->getValue(state, arguments[1].value, value);
    assert(success && "FIXME: Unhandled solver failure");
    bool res;
    success = executor.solver->mustBeTrue(
        state, EqExpr::create(arguments[1].value, value), res);
    assert(success && "FIXME: Unhandled solver failure");
    if (res) {
      llvm::errs() << " == " << value;
    } else { 
...
void SpecialFunctionHandler::handleCalloc(ExecutionState &state,
  ...
  ref<Expr> size = MulExpr::create(arguments[0].value, arguments[1].value);
...
void SpecialFunctionHandler::handleRealloc(ExecutionState &state,
  ...
  if (zeroSize.first) { // size == 0
    executor.executeFree(*zeroSize.first, arguments[0], target);
  }
...
void SpecialFunctionHandler::handleDebugSubsumption(
  ...
  ref<Expr> level = executor.toUnique(state, arguments[0].value);
...
void SpecialFunctionHandler::handleDebugState(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<Cell> &arguments) {
  assert(arguments.size() == 0 &&
...
void SpecialFunctionHandler::handleDebugStateOff(ExecutionState &state,
                                                 KInstruction *target,
                                                 std::vector<Cell> &arguments) {
  assert(arguments.size() == 0 &&
...
void SpecialFunctionHandler::handleMakeSymbolic(ExecutionState &state,
  ...
  if (arguments.size() == 2) {
    ...
    name = readStringAtAddress(state, arguments[2].value);
  }
  ...
  for (Executor::ExactResolutionList::iterator it = rl.begin(), 
    ...
    bool success __attribute__((unused)) = executor.solver->mustBeTrue(
        *s, EqExpr::create(ZExtExpr::create(arguments[1].value,
